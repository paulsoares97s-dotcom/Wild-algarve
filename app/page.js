...
export default function WildAlgarve() {
  ...
  useEffect(() => {
    ...
    const isIG = ua.includes("Instagram") || ua.includes("FBAN") || ua.includes("FBAV") || ua.includes("FB_IAB") || ua.includes("FBIOS");
    setIsInstagram(isIG);

    if (isIG) {
      document.documentElement.style.setProperty("--ig-mode", "1");
    }
    ...
    const onScroll = () => {
      ...
      if (hero) setStickyVisible(hero.getBoundingClientRect().bottom < 100);
    };
    onResize(); onScroll();
  ...
  useEffect(() => {
    const t = setInterval(() => setActiveGallery(p => (p + 1) % 3), 5000);
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      else setBookingError("Something went wrong. Please contact us on WhatsApp.");
    } catch {
  ...
  // ─── GALLERY PHOTOS — Les nouvelles photos ChatGPT ──────
  const galleryPhotos = [
    { src: "/v3-photo-action.jpeg", title: "The ride", caption: "Wind in your hair. Smiles all the way." },
    { src: "/v3-photo-cascade.jpeg", title: "The plunge", caption: "Where the heat fades into turquoise." },
    { src: "/v3-photo-sunset.jpeg", title: "The golden hour", caption: "A glass of wine, an ocean of orange." },
  ];

  // Instagram-specific styles
  const heroHeight = isInstagram
    ? (isMobile ? "min(78vh, 78dvh)" : "92vh")
    : (isMobile ? "78vh" : "92vh");
    ...
    <div style={{
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;-webkit-font-smoothing:antialiased;-webkit-tap-highlight-color:transparent}
        ...
        body{-webkit-overflow-scrolling:touch;overscroll-behavior:none;overflow-x:hidden}
        ...
        /* Hero image stable - prevent Instagram zoom bug */
        .hero-img{position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;display:block;backface-visibility:hidden;-webkit-backface-visibility:hidden}
        /* Sticky CTA fix Instagram */
        .sticky-cta{padding-bottom:calc(12px + env(safe-area-inset-bottom, 0px))}
        /* Gallery hover */
        .gallery-card{transition:transform .6s cubic-bezier(.16,1,.3,1)}
      ...
      <nav style={{
        ...
        <div onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} style={{ cursor: "pointer" }}>
          <img src="/v3-logo.png" alt="Wild-Algarve" style={{ height: scrolled ? "28px" : (isMobile ? "32px" : "38px"), width: "auto", transition: "height .4s ease", display: "block" }} />
        </div>
        {!isMobile ? (
          ...
          <button onClick={() => setMenuOpen(true)} aria-label="Menu" style={{ display: "flex", flexDirection: "column", gap: 5, background: "rgba(255,255,255,0.18)", border: "1px solid rgba(255,255,255,0.3)", cursor: "pointer", padding: 0, width: 38, height: 38, borderRadius: 12, justifyContent: "center", alignItems: "center" }}>
      ...
      {menuOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 999, background: `linear-gradient(135deg,${C.brown},${C.brownDeep})`, display: "flex", flexDirection: "column" }}>
          <div style={{ padding: "14px 5%", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <button onClick={() => setMenuOpen(false)} aria-label="Close" style={{ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.3)", width: 38, height: 38, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", color: C.white, fontSize: "1.4rem", cursor: "pointer", padding: 0 }}>×</button>
          </div>
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 5%", gap: 4 }}>
            {NAV.map((n, i) => (
              <a key={n.label} className="mlink" onClick={() => goTo(n.target)} style={{ fontFamily: "'Archivo',sans-serif", fontSize: "2.2rem", fontWeight: 800, color: C.cream, cursor: "pointer", letterSpacing: "-0.02em", padding: "10px 0", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid rgba(255,255,255,0.08)`, animationDelay: `${0.05 + i * 0.05}s` }}>
      ...
      <section data-hero id="trip" style={{ position: "relative", overflow: "hidden", background: C.brown }}>
        <div style={{ position: "relative", width: "100%", height: heroHeight, overflow: "hidden" }}>
          <picture>
            <source media="(min-width:768px)" srcSet="/v3-hero-desktop.jpeg" />
            <img className="hero-img" src="/v3-hero-mobile.jpeg" alt="Wild Algarve UMM Jeep" style={{ objectPosition: "center 60%" }} fetchpriority="high" />
          </picture>
      ...
      {/* INSIDE THE JEEP — Refonte agence */}
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 28 : 56, alignItems: "center" }}>
            <div>
              <div style={{ position: "relative", overflow: "hidden", aspectRatio: "4/3", borderRadius: 16, boxShadow: "0 8px 32px rgba(61,35,20,.12)", marginBottom: 16 }}>
                <img src="/v3-inside.jpeg" alt="Inside the UMM jeep" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} loading="lazy" />
              </div>
            ...
            <div>
              ...
              <p style={{ fontSize: isMobile ? "1rem" : "1.1rem", lineHeight: 1.75, color: C.brownMid, marginBottom: 36 }}>Top down, dirt roads, hidden waterfalls. Just you, a small group of mates, and a local guide who knows where to find the good stuff. 3h30 that fly by.</p>
              
              {/* STATS — Style éditorial premium */}
              <div style={{ borderTop: `1px solid ${C.sand}`, borderBottom: `1px solid ${C.sand}`, padding: "28px 0", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: isMobile ? 8 : 16 }}>
                {[
                  { v: "3h30", l: "Duration" },
                  { v: "7", l: "Max guests" },
                  { v: "4+", l: "Min age" },
                  ...
                  <div key={i} style={{ textAlign: "left", paddingRight: i < 3 ? (isMobile ? 4 : 12) : 0, borderRight: i < 3 ? `1px solid ${C.sand}` : "none" }}>
                    <div style={{ fontFamily: "'Archivo',sans-serif", fontSize: isMobile ? "1.3rem" : "1.7rem", fontWeight: 800, color: C.brown, lineHeight: 1, letterSpacing: "-0.02em", marginBottom: 4 }}>{s.v}</div>
                    <div style={{ fontFamily: "'Archivo',sans-serif", fontSize: isMobile ? "0.55rem" : "0.65rem", fontWeight: 600, color: C.brownMid, letterSpacing: "0.14em", textTransform: "uppercase" }}>{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* GALLERY — Nouvelle section éditoriale */}
      <section id="gallery" style={{ background: C.cream, padding: isMobile ? "64px 0 80px" : "100px 0" }}>
        <div style={{ padding: isMobile ? "0 20px" : "0 8%", marginBottom: isMobile ? 32 : 48, maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
            <span style={{ width: 22, height: 1.5, background: C.terracotta, borderRadius: 2, display: "inline-block" }} />
            <span style={{ color: C.terracotta, fontFamily: "'Archivo',sans-serif", fontSize: "0.68rem", letterSpacing: "0.22em", textTransform: "uppercase", fontWeight: 600 }}>The moments</span>
          </div>
          <h2 style={{ fontFamily: "'Archivo',sans-serif", fontWeight: 800, fontSize: isMobile ? "clamp(38px,10vw,80px)" : "clamp(48px,7vw,92px)", lineHeight: 0.95, letterSpacing: "-0.03em", color: C.brown }}>
            Three hours.<br/><em style={{ color: C.terracotta, fontStyle: "italic" }}>One trip.</em>
          </h2>
        </div>

        {/* Mosaïque desktop / Slider mobile */}
        {!isMobile ? (
          <div style={{ padding: "0 8%", maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gridTemplateRows: "auto auto", gap: 12, gridTemplateAreas: '"a b c" "a d c"' }}>
            {galleryPhotos.map((p, i) => (
              <div key={i} className="gallery-card" style={{ gridArea: ["a","b","c"][i], position: "relative", overflow: "hidden", borderRadius: 16, aspectRatio: i === 0 ? "1/1" : "1/1.1", boxShadow: "0 4px 24px rgba(61,35,20,.08)" }}>
                <img src={p.src} alt={p.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} loading="lazy" />
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "32px 24px 20px", background: "linear-gradient(to top,rgba(0,0,0,0.7),transparent)" }}>
                  <div style={{ fontFamily: "'Archivo',sans-serif", fontSize: "0.6rem", fontWeight: 600, color: C.ochre, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: 6 }}>0{i+1}</div>
                  <div style={{ fontFamily: "'Archivo',sans-serif", fontSize: "1.3rem", fontWeight: 800, color: C.white, letterSpacing: "-0.02em", lineHeight: 1.1, marginBottom: 4 }}>{p.title}</div>
                  <div style={{ fontSize: "0.85rem", color: "rgba(255,255,255,0.85)", lineHeight: 1.4 }}>{p.caption}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ position: "relative", width: "100%", aspectRatio: "4/5", overflow: "hidden" }}>
            {galleryPhotos.map((p, i) => (
              <div key={i} className="slide" style={{ opacity: activeGallery === i ? 1 : 0 }}>
                <img src={p.src} alt={p.title} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} loading="lazy" />
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "48px 20px 28px", background: "linear-gradient(to top,rgba(0,0,0,0.75),transparent)" }}>
                  <div style={{ fontFamily: "'Archivo',sans-serif", fontSize: "0.62rem", fontWeight: 600, color: C.ochre, letterSpacing: "0.22em", textTransform: "uppercase", marginBottom: 8 }}>0{i+1} · {["The ride","The plunge","Golden hour"][i]}</div>
                  <div style={{ fontFamily: "'Archivo',sans-serif", fontSize: "1.5rem", fontWeight: 800, color: C.white, letterSpacing: "-0.02em", lineHeight: 1.15, maxWidth: 320 }}>{p.caption}</div>
                </div>
              </div>
            ))}
            <div style={{ position: "absolute", bottom: 16, right: 20, display: "flex", gap: 6, zIndex: 5 }}>
              {galleryPhotos.map((_, i) => (
                <button key={i} onClick={() => setActiveGallery(i)} aria-label={`Photo ${i+1}`} style={{ width: activeGallery === i ? 24 : 8, height: 8, borderRadius: 4, border: "none", background: activeGallery === i ? C.white : "rgba(255,255,255,0.5)", cursor: "pointer", transition: "all .3s ease", padding: 0 }} />
              ))}
            </div>
          </div>
        )}
      ...
      <section id="route" style={{ background: C.cream, paddingBottom: isMobile ? 64 : 100, overflow: "hidden" }}>
        <div style={{ padding: isMobile ? "0 20px" : "0 8%", marginBottom: isMobile ? 28 : 48, maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
            <span style={{ width: 22, height: 1.5, background: C.terracotta, borderRadius: 2, display: "inline-block" }} />
        ...
        <div style={{ position: "relative", width: "100%", marginBottom: isMobile ? 32 : 56, overflow: "hidden", aspectRatio: "21/9" }}>
          <img src="/v3-waterfall.jpeg" alt="Hidden waterfall" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} loading="lazy" />
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                    <div>
                      ...
                      <input type="text" autoComplete="given-name" value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="John" style={inputStyle} />
                    </div>
                    <div>
                      <label style={labelStyle}>Last name</label>
                      <input type="text" autoComplete="family-name" value={lastName} onChange={e => setLastName(e.target.value)} placeholder="Doe" style={inputStyle} />
                    </div>
                  </div>
                  <div style={{ marginBottom: 12 }}>
                    <label style={labelStyle}>Email address</label>
                    <input type="email" inputMode="email" autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="john@email.com" style={inputStyle} />
                  </div>
                  <div style={{ marginBottom: 20 }}>
                    <label style={labelStyle}>Phone (WhatsApp)</label>
                    <input type="tel" inputMode="tel" autoComplete="tel" value={phone} onChange={e => setPhone(e.target.value)} placeholder="+44 777 123 456" style={inputStyle} />
                  </div>
      ...
      <section id="pricing" style={{ background: C.creamDark, padding: isMobile ? "64px 20px" : "100px 8%", textAlign: "center" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 16 : 24, maxWidth: 760, margin: "0 auto" }}>
            {[
              { tag: "All inclusive", label: "Adult", amount: "39", featured: true, items: ["3h30 off-road ride", "Pickup in Albufeira", "Local guide & stories", "Wine at the viewpoint"] },
              { label: "Under 15", amount: "25", featured: false, items: ["Same trip as the adults", "Same pickup & ride", "Juice at the viewpoint", "Min. age 4 years"] },
            ].map((p, i) => (
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "2fr 1fr 1fr", gap: isMobile ? 36 : 56, marginBottom: 36 }}>
            ...
            <div>
              ...
              <a href="mailto:wild-algarve@hotmail.com" style={{ display: "block", color: C.cream, textDecoration: "none", fontSize: "0.9rem" }}>wild-algarve@hotmail.com</a>
            </div>
            <div>
